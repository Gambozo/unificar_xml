...
foreach ($xml->imovel as $estate) {
    ...
    echo '<codigo cod="' . $codigo . '" idImv="' . $id . '">';
    ...
    foreach ($estate->disponibilidade->disponivel as $availability) {
        ...
        echo '<disponivel finalidade="' . strtoupper($finalidade) . '" valor="' . $valor . '" ' . $valorcond . '/>';
    }
    echo '</disponibilidade>';
    if ($numfotos >= 1) {
        echo '<imagens>';
    ...
    echo '<endereco><![CDATA[ ' . $endereco . ' ]]></endereco>';
    echo '<bairro><![CDATA[ ' . $bairro . ' ]]></bairro>';
